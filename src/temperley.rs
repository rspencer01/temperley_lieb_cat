...
use crate::structures::{Ring, RingOps, Signed};
use crate::tex::Tex;
use crate::TLDiagram;
    ...
    for<'r> &'r R: RingOps<&'r R, R>,
{
    ...
    where
        F: Fn(&TLDiagram) -> TLDiagram,
    {
        TLMorphism::new(
            self.coeffs.iter().map(|(k, v)| (f(k), v.clone())).collect(),
            self.delta.clone(),
        )
    }
    ...
    {
        TLMorphism::new(
            self.coeffs.iter().map(|(k, v)| (*k, f(v))).collect(),
            self.delta.clone(),
        )
    }
...
impl<R: Ring> Eq for TLMorphism<R> where for<'r> &'r R: RingOps<&'r R, R> {}
    ...
    for<'r> &'r R: RingOps<&'r R, R>,
{
    ...
    fn mul(self, other: &TLMorphism<R>) -> TLMorphism<R> {
        ...
        for (d, v) in other.coeffs.iter() {
            ...
            ans_vec.extend(self.coeffs.iter().map(|(dp, vp)| {
                let m = dp * d;
        ...
        if ans_vec.is_empty() {
            ...
            TLMorphism::new(ans_vec, Some(delta))
        }
    }
}
    ...
    for<'r> &'r R: RingOps<&'r R, R>,
{
    ...
    fn bitor(self, other: &TLMorphism<R>) -> TLMorphism<R> {
        ...
        for (d, v) in self.coeffs.iter() {
            ans_vec.extend(other.coeffs.iter().map(|(dp, vp)| (*d | dp, v * vp)));
        }
        TLMorphism::new(ans_vec, delta)
    }
}
    ...
    for<'r> &'r R: RingOps<&'r R, R>,
{
    fn from(diag: TLDiagram) -> TLMorphism<R> {
        TLMorphism::new(vec![(diag, R::one())], None)
    }
}

impl<R: Ring> RingOps<TLMorphism<R>, TLMorphism<R>> for TLMorphism<R> where
    for<'r> &'r R: RingOps<&'r R, R>
{
}
...
mod tests {
    ...
    fn add() {
        assert_eq!(
            TLMorphism::<i128>::id_zero(5) + TLMorphism::id_zero(5),
            TLMorphism::id_zero(5)
        );
        assert_ne!(
            TLMorphism::<i128>::id_zero(5) + TLMorphism::id_zero(5),
        ...
        assert_eq!(
            TLMorphism::<i128>::id_zero(5) + TLMorphism::id(5),
            TLMorphism::id(5)
        );
        assert_eq!(
            TLMorphism::<i128>::id(5) + TLMorphism::id_zero(5),
            TLMorphism::id(5)
        );
    }
    ...
    #[test]
    fn mul() {
        assert_eq!(TLMorphism::id_zero(6), TLMorphism::id(6) * (0 as i128));
