    ...
    for m in (*JW_COMPUTED.lock().unwrap() + 1)..(n + 1) {
        for d in TLDiagram::all(m, m) {
            ...
            for i in dp.simple_links().into_iter() {
                ...
                ans = ans
                    + &JW_COEFFS.lock().unwrap()[&dp.remove_cap(i)]
                        * &Fraction::new(quantum(-(i as isize)), quantum(-(m as isize)))
                        * &coeff;
            }
...
fn reduce_mod(
    ...
    let foo = a
        .coeffs
        .into_iter()
        .map(|(k, v)| (k, Fraction::new(v.num() % &p, v.den() % &p)))
        .collect::<Vec<_>>();
    TLMorphism::new(foo, a.delta)
}
